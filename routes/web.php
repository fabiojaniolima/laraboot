...
Route::prefix('painel')->middleware('auth')->group(function () {
    ...
    Route::prefix('gerenciar-usuarios')->middleware('can:only-superadmin')->group(function () {
        ...
        Route::get('mostrar/{user}', 'Panel\ManageUsers\ManageUsersController@show');
        Route::get('alterar-status/{user}', 'Panel\ManageUsers\ManageUsersController@changeStatus');
        Route::get('excluir/{user}', 'Panel\ManageUsers\ManageUsersController@destroy');
    });
});
